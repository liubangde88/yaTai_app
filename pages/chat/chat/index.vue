		...
		<view class="chat-main">
			<view class="chat-ul bgWhite flexColumn" v-if="list.length > 0">
				<view class="chat-li flexColumn" v-for="(item,index) in list" :key="index">
					...
					<!-- {{item.qrAddress}} -->
						<view class="copy-btn fs28 colf" style="flex: 1;" @click="toPath(item.qrAddress)">{{$t('message.goTo')}}</view>
					</view>
					<image :src="item.qrUrl" mode="" class="kefu-img" @click="previewImage(item.qrUrl)" v-if="item.qrUrl"></image>
				</view>
			</view>
